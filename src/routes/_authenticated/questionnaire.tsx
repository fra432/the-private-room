...
function QuestionnairePage() {
  ...
  async function submit(e: React.FormEvent) {
    e.preventDefault();
    if (!user) return;
    if (!form.hair_type.trim() || !form.hair_length.trim() || !form.hair_color.trim() || !form.goals.trim()) {
    ...
    const { error } = existingId
      ? await supabase.from("questionnaires").update(payload).eq("id", existingId)
    ...
    toast.success(existingId ? "Questionario aggiornato." : "Grazie, ora puoi prenotare.");
    navigate({ to: "/book" });
  }
    ...
    <main className="theme-interior min-h-screen bg-background text-foreground">
      ...
      <div className="mx-auto max-w-2xl px-6 py-16">
        ...
        <h1 className="mt-4 font-serif text-4xl leading-tight text-foreground md:text-5xl">
          Questionario di
          <span className="italic text-[color:var(--gold)]"> presentazione</span>
        </h1>
        ...
        {loading ? (
          <p className="mt-16 text-xs tracking-[0.4em] uppercase text-muted-foreground">…</p>
        ) : (
          <form onSubmit={submit} className="mt-12 space-y-10">
            ...
            <div className="flex items-center justify-between border-t border-[color:var(--border)] pt-8">
              <Link to="/dashboard" className="text-[0.55rem] tracking-[0.5em] uppercase text-muted-foreground hover:text-foreground">
                Annulla
              </Link>
              <button
                type="submit"
                disabled={saving}
                className="inline-flex h-12 items-center justify-center bg-foreground px-10 text-[0.6rem] tracking-[0.5em] uppercase text-background hover:opacity-90 disabled:opacity-50"
              >
                {saving ? "Salvataggio…" : existingId ? "Aggiorna" : "Salva e prenota"}
              </button>
            </div>
          </form>
        )}
...
function Field(props: { label: string; hint?: string; value: string; onChange: (v: string) => void; maxLength?: number }) {
  return (
    <label className="flex flex-col gap-2">
      <span className="text-[0.6rem] tracking-[0.4em] uppercase text-muted-foreground">{props.label}</span>
      <input
        type="text"
        value={props.value}
        onChange={(e) => props.onChange(e.target.value)}
        maxLength={props.maxLength}
        className="w-full bg-transparent border-b border-[color:var(--border)] pb-2 pt-1 text-base focus:border-[color:var(--gold)] focus:outline-none"
      />
      {props.hint && <span className="text-[11px] text-muted-foreground/80">{props.hint}</span>}
    </label>
  );
}

function Area(props: { label: string; hint?: string; value: string; onChange: (v: string) => void; required?: boolean }) {
  return (
    <label className="flex flex-col gap-2">
      <span className="text-[0.6rem] tracking-[0.4em] uppercase text-muted-foreground">{props.label}</span>
      <textarea
        ...
        className="w-full resize-none bg-transparent border-b border-[color:var(--border)] pb-2 pt-1 text-sm focus:border-[color:var(--gold)] focus:outline-none"
      />
      {props.hint && <span className="text-[11px] text-muted-foreground/80">{props.hint}</span>}
    </label>
  );
