    ...
    <main className="relative h-screen overflow-hidden bg-background text-foreground">
      ...
      <div className="relative mx-auto grid h-screen w-full max-w-6xl grid-cols-1 lg:grid-cols-2">
        ...
        <aside className="relative flex flex-col justify-between px-8 py-10 lg:border-r lg:border-[color:var(--gold)]/15 lg:px-14 lg:py-16 h-screen overflow-hidden">
          <Link to="/welcome" className="group inline-flex items-center gap-3 self-start text-[0.6rem] tracking-[0.5em] uppercase text-muted-foreground transition-colors hover:text-[color:var(--gold)]">
            <BackArrow />
            Indietro
          </Link>
...
function Field({
  ...
  return (
    <label className="flex flex-col gap-2">
      <span className="text-[0.7rem] tracking-[0.5em] uppercase text-foreground/60 font-medium">{label}</span>
      {as === "textarea" ? (
